...
(provide crack-cipher
         ;; For extra credit (this part of the assignment may not be released eventually).
...
;;                                                                                  ;;
;; The function must return a list of 26 lower-case chars representing the key. If  ;;
;; you are unable to completely determine the key, return a partial key.            ;;
;; A trivial partial key is a list of 26 underscores.                               ;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(define blank-key (build-list 26 (lambda (_) #\_)))
(define (crack-cipher strategies key) ;; Returns a list of chars, the encryption key.
  ;; make use of `utils:ciphertext` and `utils:cipher-word-list`
(define (main-helper subs-lst key)
  (cond[(null? subs-lst) '()]
       [else  (remove* (list #f)
                      (let ([p (algo:dictionary-closure (utils:add-substitution (car subs-lst) key))])
                        (if p (cons (algo:secret-word-enumeration p) (main-helper (cdr subs-lst) key))
                            (cons p (main-helper (cdr subs-lst) key)))))]))
              
  (let ([final-ans (main-helper ((car strategies) key) key)])
    (if (null? final-ans) (algo:secret-word-enumeration (algo:dictionary-closure blank-key))
        final-ans)))

;; Ignore this in case the assignment has no extra credit tasks.  Otherwise, the
;; behaviour is exactly as `crack-cipher` except that you cannot use
;; `secret-word-enumeration`.
(define (crack-hard-cipher strategies key) ;; Returns a list of chars, the encryption key.
  ;; make use of `utils:ciphertext` and `utils:cipher-word-list`
  key)
